...
def inspect_original_IMU(normalize=False, unit='si'):
    ...
    config = get_config_universal('mosurf', config_path=None)
    dataset_handler = DataSet(config, load_dataset=True)
    x      = dataset_handler.x
    y      = dataset_handler.y
    labels = dataset_handler.labels

    subject_list     =     subject_list    = ["AMOAS01","AMOAS02","AMOAS03","AMOAS04","AMOAS05","AMOAS06","AMOAS07","AMOAS08","AMOAS09","AMOAS010","AMOAS011","AMOAS012","AMOAS013","AMOAS014","AMOAS015","AMOAS016","AMOAS017","AMOAS018","AMOAS019"]

    selected_sensors = config["selected_sensors"][:3]
    sensor_to_plot = [0,1,2]  # ← hier auswählen welche Sensoren
    sensor_names     = config["selected_sensors"]
    n_features       = len(config["selected_imu_features"])  # 6

    #speed    = 'N'
    movement = 'picking_up_pen'
    ...
    for s_idx, sensor_id in enumerate(selected_sensors):
        ...
        sensor_label = sensor_names[s_idx] if s_idx < len(sensor_names) else f"Sensor {sensor_id}"
        ...
        for group_idx, subject_group in enumerate(subject_groups):
            ...
            #fig_acc.suptitle(f"ACC{norm_str}  |  {sensor_label}  |  {movement}  |  Group {group_idx+1}",
            #                 fontsize=13, fontweight="bold")
            #fig_gyro.suptitle(f"GYRO{norm_str}  |  {sensor_label}  |  {movement}  |  Group {group_idx+1}",
            #                  fontsize=13, fontweight="bold")
        
            for subject in subject_group:  # ← nur Subjects dieser Gruppe
                for i in range(len(labels)):
                    if (labels[i]['subject'][0] == subject and      # ← exakter Subject-Match
                        labels[i]['movement'][0] == movement):
                        ...
                        subject_display = subject.replace("AMOAS0", "P")  # "subject_1" → "Subject 1"
            ...
            save_dir = '/home/benedikt_nothhelfer/ba_upki_benedikt/projects/UpKi_BioMat_Surf/results/compareplots/IMU'
            os.makedirs(save_dir, exist_ok=True)
            fname_acc  = f"ACC_{sensor_label}_{movement}_group{group_idx+1}.svg"
            fname_gyro = f"GYRO_{sensor_label}_{movement}_group{group_idx+1}.svg"
...
def inspect_original_Angles(normalize=False):
    ...
    subject_list    = ["AMOAS01","AMOAS02","AMOAS03","AMOAS04","AMOAS05","AMOAS06","AMOAS07","AMOAS08","AMOAS09","AMOAS010","AMOAS011","AMOAS012","AMOAS013","AMOAS014","AMOAS015","AMOAS016","AMOAS017","AMOAS018","AMOAS019"]
    ...
    for s_idx, angle_name in enumerate(selected_angles):  # ← Fix: enumerate korrekt
        ...
        for group_idx, subject_group in enumerate(subject_groups):  # ← Fix: Einrückung
            ...
            #fig.suptitle(f"Angle{norm_str}  |  {angle_name}  |  {movement}  |  Group {group_idx+1}",fontsize=13, fontweight="bold")
            ...
            for subject in subject_group:
                for i in range(len(labels)):
                    if (labels[i]['subject'][0] == subject and
                        ...
                        subject_display = subject.replace("AMOAS0", "P")
            ...
            display_name = JOINT_DISPLAY_NAMES_SURF.get(angle_name, f"{angle_name} [°]")
            ...
            save_dir = '/home/benedikt_nothhelfer/ba_upki_benedikt/projects/UpKi_BioMat_Surf/results/compareplots/Angles'
            os.makedirs(save_dir, exist_ok=True)
            fname = f"Angle_{angle_name}_{movement}_group{group_idx+1}.svg"
