...
class DataSet:
    ...
    def _preprocess(self):
        ...
        #cb_count = sum(1 for label in self.labels if label['movement'].iloc[0] == 'CB')
        #print(f"CB trials after remove_outlier: {cb_count}")
        if self.resample:
            self.x, self.y, self.labels = self.run_resample_signal(self.x, self.y, self.labels)#labels nicht verwendet
            
        if self.config['opensim_filter']:
            filteropensim_handler = FilterOpenSim(self.y, lowcut=6, fs=100, order=2)
            self.y = filteropensim_handler.run_lowpass_filter() #labels nicht verwendet
            
        if self.config['imu_filter']:
            filterimu_handler = FilterIMU(self.x, lowcut=10, fs=100, order=2)
            self.x = filterimu_handler.run_lowpass_filter() #labels nicht verwendet
    ...
    def run_dataset_split_loop(self): #label 'TrialType2' wird aufgerufen. Kann aber ausgeklammert werden
        train_labels = []
        test_labels = []
        train_x = []
        train_y = []
