...
def run_main(model_name=None, overlap=None, run_idx=18):
    ...
    #config = get_config_universal('mosurf', config_path=config_path)


    #config = get_config_universal('mosurf', config_path=args.config)
    # für finalen 10runs
    if model_name == 'transformertsai':
        config = get_config_universal('mosurf', config_path='configs/mosurf_config_transformer.json')
    elif model_name == 'bilstm':
        config = get_config_universal('mosurf', config_path='configs/mosurf_config_bilstm.json')
    else:
        config = get_config_universal('mosurf', config_path='configs/mosurf_config_cnnlstm.json')
    ...
    #war nur für testing. an sich unnötig
    ...
    for fold_idx, test_subject in enumerate(all_subjects):
        ...
        for activity in config['test_activity']:
            ...
            if collector is not None:
                ...
                collector.log_predictions(subject=test_subject, activity=activity,y_true=y_true[activity_index], y_pred=y_pred[activity_index],
                labels = kihadataset_test['labels'][activity_index], joint_names=config['selected_opensim_labels'])
...
def run_standard(config):
    ...
    if tuning == True:
        ...
        if config['model_name'] == 'linear':
            ...
            if train_dataset.y_scaler is not None:
                shape = y_pred.shape
                y_pred = train_dataset.y_scaler.inverse_transform(y_pred.reshape(-1, shape[2])).reshape(shape)
                y_true = train_dataset.y_scaler.inverse_transform(y_true.reshape(-1, shape[2])).reshape(shape)
            # Evaluation
            if individual_plot:
...
if __name__ == '__main__':
    ...
    for model_name in models:
        for run_idx in range(args.run_idx_start, 10):
            ...
            #run_main(model_name=model_name, overlap=0.3)