...
GDRIVE_REMOTE = GDRIVE_REMOTE_SURF
...
# ULTra-MoCap metadata 

ACTIVITY_LABELS_ULTRA = {
    "AS":  "Arm Swing",
    "CB":  "Crossbody Reach",
    "EF":  "Elbow Flexion",
...
ACTIVITY_LABELS_SURF = {
    "picking_up_pen":  "Picking Up Pen",
    "shelve_ordering": "Shelve Ordering",
}

# MoSurf has no speed dimension
ACTIVITY_SPEED_ORDER_SURF = {
    "picking_up_pen":  ["trial"],
    "shelve_ordering": ["trial"],
}

SPEED_DISPLAY_LABELS_SURF = {
    "trial": "",
}

JOINT_LABELS_SURF = {
    "shoulder_elv_r":  "Shoulder Elevation R [°]",
    "shoulder_rot_r":  "Shoulder Rotation R [°]",
    "elv_angle_r":     "Elevation Angle R [°]",
    "elbow_flexion_r": "Elbow Flexion R [°]",
    "pro_sup_r":       "Forearm Pro/Supination R [°]",
    "shoulder_elv_l":  "Shoulder Elevation L [°]",
    "shoulder_rot_l":  "Shoulder Rotation L [°]",
    "elv_angle_l":     "Elevation Angle L [°]",
    "elbow_flexion_l": "Elbow Flexion L [°]",
    "pro_sup_l":       "Forearm Pro/Supination L [°]",
}
...
def run_main():
    ...
    pred_path = "/home/benedikt_nothhelfer/ba_upki_benedikt/projects/UpKi_BioMat_Surf/results/predictions_all/predictions_lopo__sUpperThoracic-LeftUpperArm-LeftForearm-RightUpperArm-RightForearm__pad256_lr001_o7__imuFilt_osimFilt_noYScal_byVar_run18"
    ...
    baseline_metrics_csv = os.path.join(results_dir,"metrics_all", f"metrics_surf_30.3_baseline.csv")
...
#--temporär für visualisieren von finalen 10runs
    settings = "3_lopo__sUpperThoracic-LeftUpperArm-LeftForearm-RightUpperArm-RightForearm__pad256_lr001_o3__imuFilt_osimFilt_noYScal_byVar"
...
#--temporär
    ...
    if comparison_table == False:
        ...
        if comparison_table_vertical == True:
            build_comparison_excel_vertical(baseline_metrics_csv, df, excel_out, settings,label_a="ultra_baseline" )
        else:
            build_comparison_excel_horizontal(baseline_metrics_csv, metrics_csv, excel_out, settings,label_a="ultra_baseline" )
    

    build_line_plots(pred_dir, plots_dir, settings, dataset_name=dataset_name)


#Dataset-aware helpers

def _get_activity_labels(dataset_name: str) -> dict:
    return ACTIVITY_LABELS_SURF if dataset_name == "mosurf" else ACTIVITY_LABELS_ULTRA

def _get_joint_labels(dataset_name: str) -> dict:
    return JOINT_LABELS_SURF if dataset_name == "mosurf" else JOINT_LABELS_ULTRA

def _get_speed_order(dataset_name: str) -> dict:
    return ACTIVITY_SPEED_ORDER_SURF if dataset_name == "mosurf" else ACTIVITY_SPEED_ORDER_ULTRA

def _get_speed_display(dataset_name: str) -> dict:
    return SPEED_DISPLAY_LABELS_SURF if dataset_name == "mosurf" else SPEED_DISPLAY_LABELS_ULTRA

def _has_speed_dim(dataset_name: str) -> bool:
    """MoSurf trial files contain no meaningful speed token."""
    return dataset_name != "mosurf"
...
def aggregate_runs(pred_dir_base, settings,  dataset_name="mosurf"):
    """
...
def _build_sheet2(wb, df, algos, subjects, activities, metrics_csv, label, col_offset=0, row_offset=0):
    ...
    for ai, activity in enumerate(activities):
        ...
        act_label = ACTIVITY_LABELS_SURF.get(activity, activity)
...
def _build_sheet3(wb, df, algos, subjects, activities, joints, metrics_csv, label, col_offset=0, row_offset=0):
    ...
    for ji, joint in enumerate(joints):
        ...
        joint_label = JOINT_LABELS_SURF.get(joint, joint)
...
def build_excel_table(metrics_csv, excel_out, dataset_name="ultramocap"):
    if not os.path.exists(metrics_csv):
        print(f"[ERROR] Keine metrics.csv gefunden unter {metrics_csv}")
        return

    df         = pd.read_csv(metrics_csv)
    df["subject"] = df["subject"].str.replace("AMOAS", "Participant ", regex=False)
    ...
    _build_sheet2(wb, df, algos, subjects, activities, dataset_name)
    _build_sheet3(wb, df, algos, subjects, activities, joints, dataset_name)
...
def build_comparison_excel_vertical(metrics_csv_a, metrics_csv_b, excel_out, settings, label_a):
    """
    ...
    # Sheet 1
    _build_sheet1(wb, df_b, algos, subjects, metrics_csv_b, settings)
    ...
    # Sheet 2
    _build_sheet2(wb, df_b, algos, subjects, activities, metrics_csv_b, settings)
    ...
    # Sheet 3
    _build_sheet3(wb, df_b, algos, subjects, activities, joints, metrics_csv_b, settings)
...
#  Line Plots
def build_line_plots(pred_dir, plots_dir, fileinfo, mode: str = "all", dataset_name: str = "ultramocap"):
    """
    ...
    speed_order  = _get_speed_order(dataset_name)

    if mode == "fast":
        fast_combos = set()
        for (subject, activity, speed) in combos_set:
            order            = speed_order.get(activity, [])
...
def _plot_joint(plots_dir, subject, activity, speed, joint, dfs, mode, dataset_name="ultramocap"):
    ...
    for h, l in zip(handles, labels):
        if l not in seen:
            seen[l] = h
    ax.legend(seen.values(), seen.keys(),loc="upper right", fontsize=10, framealpha=0.95, edgecolor="#CCCCCC")

    # Title
    activity_labels = _get_activity_labels(dataset_name)
    joint_labels    = _get_joint_labels(dataset_name)
    speed_display   = _get_speed_display(dataset_name)

    subj_label = subject.replace("subject_", "Participant ").replace("AMOAS", "Participant ")
    act_label   = activity_labels.get(activity, activity)
    joint_label = joint_labels.get(joint, joint)
    speed_label = speed_display.get(speed, speed)

    has_speed = _has_speed_dim(dataset_name) and mode == "all" and speed_label
    if has_speed:
    ...
    display_name = JOINT_LABELS_SURF.get(joint, f"{joint} [°]")
...
def compute_metrics_from_predictions(pred_dir, dataset_name="mosurf"):
