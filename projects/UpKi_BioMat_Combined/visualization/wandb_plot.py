...
def wandb_plotly_true_pred(y_true, y_pred, labels, val_or_test):
    """
    Plots one random sample per joint angle: Ground Truth (blue) vs. Predicted (red).
    """
    if len(y_true) == 0:
        print(f"[wandb_plotly_true_pred] Skipping '{val_or_test}': y_true is empty.")
        return

    i = random.choice(list(range(len(y_true))))

    layout = dict(
        plot_bgcolor='rgba(0,0,0,0)',
        paper_bgcolor='rgba(0,0,0,0)',
        xaxis=dict(title='Time Steps', showgrid=False, showline=True, ticks='outside', mirror=True),
    ...
    if len(y_true.shape) == 2:
        ...
        fig.add_trace(go.Scatter(
            x=np.arange(len(y_true[i])), y=y_true[i],
            line=dict(color='blue', width=2), name='Ground Truth'))
        fig.add_trace(go.Scatter(
            x=np.arange(len(y_pred[i])), y=y_pred[i],
            line=dict(color='red', width=2, dash='dash'), name='Predicted'))
        fig.update_layout({**layout, 'title': f'Sample {i}'})
        wandb.log({val_or_test + '_label_' + labels: {'chart': fig}})
    else:
        for j in range(y_true.shape[2]):
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=np.arange(len(y_true[i, :, j])), y=y_true[i, :, j],
                line=dict(color='blue', width=2), name='Ground Truth'))
            fig.add_trace(go.Scatter(
                x=np.arange(len(y_pred[i, :, j])), y=y_pred[i, :, j],
                line=dict(color='red', width=2, dash='dash'), name='Predicted'))
            fig.update_layout({**layout, 'title': f'{labels[j]}  |  Sample {i}'})
