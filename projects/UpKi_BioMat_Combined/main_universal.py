...
def run_main():
    ...
    #config = get_config_universal('combined', config_path=None)
    ...
    config = get_config_universal('combined', config_path=args.config)
    
    #Anpassung des init wandb run. Kein zufälliger Name mehr in wandb
    extra_info = "corrected_sensorandangles"  #anpassen vor jedem Run
    run_name = f"{config['training_style']}_{config['model_name']}_ep{config['n_epoch']}_{extra_info}"
    #starte neuen wandb Run
    wandb.init(project='UpKi_Biomat_ULTra', name=run_name, config=config)
...
def run_lopo(config):
    ...
    for fold_idx, test_subject in enumerate(all_subjects):
        ...
        model_file = (config['model_name'] + '_lopo_' + test_subject + '_' +
                      "".join(config['model_train_activity']) + '.pt')
...
def run_standard(config):
    ...
    if tuning == True:
        ...
        if config['model_name'] == 'linear':
            ...
            test_dataset = DataSetBuilder(kihadataset_test['x'], kihadataset_test['y'], kihadataset_test['labels'],transform_method=config['data_transformer'],
