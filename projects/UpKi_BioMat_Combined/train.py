...
class Train:
    ...
    def run_training(self):
        ...
        for epoch in range(self.config['n_epoch']):
            ...
            if self.test_dataloader:
                if self.config.get('training_style') != 'lopo':
                    val_pred, val_target, val_loss = self.validator(self.model, self.test_dataloader, self.criterion, self.device, self.epoch)
                    Evaluation.log_epoch(self.config, val_pred, val_target,epoch=self.epoch + 1 + self.epoch_offset,train_loss=train_loss,val_loss=val_loss)
    ...
    def training(self, model, train_dataloader, optimizer, criterion, device):
        ...
        for batch_i, (x, y) in enumerate(train_dataloader):
            #Input-Batch: IMU Daten
            x = x.to(device).float()
            #Target-Batch: Gelenkwinkel
            y = y.to(device)
            ...
            #Berechnet MSE zwischen zwischen Prediction und Ground Truth
            loss = criterion(y_pred, y)
            #Backpropagation
            #1. Alte Gradienten löschen 
            #2. Neue Gradienten berechnen
            #3. Gewichte anpassen
            optimizer.zero_grad() #Gradienten vom letzten Batch löschen (sonst werden sie addiert → falsche Werte)
            loss.backward() #Berechnet wie sehr jedes Gewicht zum Loss beigetragen hat (Kettenregel / Backpropagation durch das ganze Netz)
            optimizer.step() #Adam-Optimizer verschiebt alle Gewichte ein kleines Stück in die Richtung die den Loss reduziert
    ...
    def validating(self, model, test_dataloader, criterion, device, epoch):
        ...
        #wandb.log({"Validation Loss": test_loss}, step=epoch + 1)
