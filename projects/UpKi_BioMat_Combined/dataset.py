...
class DataSet:
    def __init__(self, config, load_dataset=True):
        ...
        if load_dataset:
            ...
            self.train_activity_speed = config['train_activity_speed']
            self.test_activity_speed = config['test_activity_speed']
    ...
    def load_dataset(self):
        ...
        #getdata_handler = LoadH5DataSet(self.config) #umgeändert weil wir für Ultramocap jetzt auch eine Pickle datei erstellen
        x, y, labels = getdata_handler.run_get_dataset()
        self.x, self.y, self.labels = x, y, labels
        #self.x, self.y, self.labels = self.run_activity_based_filter(x, y, labels) # unnecessary for another dataset
        # DIAGNOSE Schritt 1: Rohdaten nach dem Laden
    
        self._preprocess()

    def _preprocess(self):
        #self.x, self.y, self.labels = remove_outlier(self.x, self.y, self.labels) #labels nicht verwendet
    ...
    def run_segmentation(self, x, y, labels):
        ...
        if self.segmentation_method == 'fixedwindow':
            segmentation_handler = FixWindowSegmentation(x, y, labels, winsize=self.config['target_padding_length'], overlap=self.config['overlap'])
    ...
    def run_dataset_split_loop(self): #sortiert Daten aus
        train_labels = []
        test_labels = []
        train_x = []
        train_y = []
        ...
        for t, trial in enumerate(self.labels):
            if all(trial['subject'].isin(self.train_subjects)) and all(trial['movement'].isin(self.train_activity)) and all(trial['speed'].isin(self.train_activity_speed)):
                train_labels.append(trial)
                train_x.append(self.x[t])
                train_y.append(self.y[t])

            elif all(trial['subject'].isin(self.test_subjects)) and all(trial['movement'].isin(self.test_activity)) and all(trial['speed'].isin(self.test_activity_speed)):
