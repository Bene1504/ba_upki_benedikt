...
class BiLSTMModel(nn.Module):
    def __init__(self, config):
        ...
        '''hardcoded auf drei Ausgänge für biomat
        if self.n_output == 1:
        ...
        #dynamische Lösung: ein LinearLayer pro Output
        self.fc2_layers = nn.ModuleList([
            nn.Linear(self.output_size1, self.output_size2) for _ in range(self.n_output)
        ])
    ...
    def forward(self, x):
        ...
        if self.n_output == 1:
            ...
            out = [out1, out2, out3]
        '''
        #dynamische Lösung: für jeden Output einen ReLU + Layer
        outputs = []
        for fc2 in self.fc2_layers:
            o = F.relu(out)
            o = fc2(o)
            outputs.append(o)
        return torch.stack(outputs, dim=0).permute(1, 2, 0)