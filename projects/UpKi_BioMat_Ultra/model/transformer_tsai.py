...
class TransformerTSAI(Module):
    ...
    def create_multiheads(self, c_out, output_size1, output_size2):
        ...
        self.fc2_layers = nn.ModuleList([
            nn.Linear(output_size1, output_size2) for _ in range(c_out)
        ])
    ...
    def forward(self, x:Tensor, mask:Optional[Tensor]=None) -> Tensor:  # x: [bs x nvars x q_len]
        ...
        '''out = self.fc_dropout(z) wieder dynamische Lösung gewählt
        if self.c_out == 1:
            out1 = F.relu(out)
        ...
        for fc2 in self.fc2_layers:
            ...
            o = fc2(o)
            outputs.append(o)
        regression_out = torch.stack(outputs, dim=0).permute(1, 2, 0)
        return regression_out
