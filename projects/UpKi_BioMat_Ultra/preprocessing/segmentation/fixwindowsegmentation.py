...
class FixWindowSegmentation:
    ...
    def fixsize_sliding_window(self, data):
        step = round((1 - self.overlap) * self.winsize)
        
        # Fall: Daten kürzer als Fenstergröße → padden
        if len(data) < self.winsize:
        ...
        for i in range(0, numOfChunks * step, step):
            chunk = data[i:i + self.winsize]
            data_out.append(chunk)
        
        '''# Letztes Fenster: prüfen ob Ende der Daten noch nicht erfasst wurde
        last_end = (numOfChunks - 1) * step + self.winsize
        if last_end < len(data):
            last_chunk = data[len(data) - self.winsize:len(data)]
            data_out.append(last_chunk)'''
